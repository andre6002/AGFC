...
$query2 = "SELECT * FROM $modulo ORDER BY id DESC LIMIT 1";
$id = my_query($query2);


$linkAtualizado = $_FILES['link'] . $id;
$queryUpdate = "UPDATE $modulo SET link = '$linkAtualizado' WHERE id = $id";
$resultUpdate = my_query($queryUpdate);
if (!$arrConfig['production'] && !$resultUpdate) {
  die ("ERRO ao atualizar link: " . $queryUpdate);
}
