...
$query2 = "SELECT stocks.id AS stock_id, socio.nome AS socio_nome, clube.nome AS clube_nome, socio.epoca AS epoca
          FROM stocks
          INNER JOIN socio ON stocks.id = socio.id
          INNER JOIN clube ON socio.id_clube = clube.id
          WHERE stocks.id = '$idStock'";

$resultado = my_query($query2);

// Verificar se um resultado foi encontrado
if (count($resultado) > 0) {
  $linha = $resultado[0]; // Assume que só haverá um resultado
  $nomeClubesocio = $linha['clube_nome'] . ' - ' . $linha['socio_nome'] . ' ' . $linha['epoca'];
}
      ...
      <div class="col-xl-4 col-lg-5 col-md-6 d-flex flex-column mx-auto"
        ...
        <div class="card card-plain mt-8" style="margin-top: 0px !important;">
          <div class="card-header pb-0 text-left bg-transparent">
            ...
            <h6 style="margin-left: 6rem;" class="font-weight-bolder text-info text-gradient">
              <?php echo $nomeClubesocio; ?>
            </h6>
          </div>
          <div class="card-body"
  ...
  switch ($arrInfo['tipo']) {
    ...
    case 'select':
      ...
      if ($campo == 'id' && isset ($arrInfo['carrega_opcoes'])) {
        ...
        $query = "SELECT p.id, p.nome as socio_nome, c.nome as clube_nome FROM $tabela p
        ...
        foreach ($arrResultados as $linha) {
          $id = $linha['id'];
          $legenda = $linha['clube_nome'] . ' - ' . $linha['socio_nome']; // Combinar nome do clube e nome do socio
