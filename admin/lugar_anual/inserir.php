...
function mostraCamposInserir($campo, $arrInfo)
{
  ...
  // if ($campo == 'fila' || $campo == 'numeroDoLugar') {
...
function carregarOpcoes($arrOpcoes)
{
  ...
  // Adiciona uma condição específica apenas para sócios
  if ($tabela == 'socios') {
    $query .= " WHERE $chave NOT IN (SELECT codSocio1 FROM lugar_anual WHERE codSocio1 IS NOT NULL)
