...
foreach ($arrCampos as $kCampos => $vCampos) {
  if (isset($vCampos['editar']) && $vCampos['editar']) {
    ...
    if ($vCampos['tipo'] == 'checkbox') {
      ...
      // Certifique-se de escapar corretamente os valores de entrada para evitar SQL Injection
      $valorEscapado = my_real_escape_string($valor); // Substitua pela sua função de escape
